// Command server 提供 LLM Studio 的 HTTP 服务：渠道配置（llm.yaml）CRUD API，
...
func main() {
	...
	e := echo.New()
	s.Register(e)
	log.Printf("LLM Studio server listening on %s (config=%s, static=%s, history=%s)", *addr, *cfgPath, abs(*staticDir), abs(*historyDir))
