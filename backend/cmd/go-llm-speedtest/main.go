// Command go-llm-speedtest 是 LLM Speed Test 的统一入口：以子命令同时支持 HTTP 服务与单次基准测试，
// 并可选把前端构建产物内嵌进同一二进制（`-tags embed` 构建）。
//
// 用法：
//
//	# 默认子命令为 server（裸跑即起 HTTP 服务）
//	go-llm-speedtest                      # 等价 go-llm-speedtest server
//	go-llm-speedtest server --addr :8787  # 起 HTTP 服务（内嵌前端或 --static 磁盘）
//	go-llm-speedtest bench --channel qianfan --model ernie-4.0-8k --prompt "用一句话介绍你自己"
//
...
import (
	...
	"github.com/SmilingXinyi/go-llm-speedtest/backend/internal/cli"
	...
	"github.com/SmilingXinyi/go-llm-speedtest/backend/internal/web"
)
...
func usage() {
	fmt.Print(`go-llm-speedtest — LLM Speed Test 统一入口

用法:
  go-llm-speedtest [server|bench] [flags]
