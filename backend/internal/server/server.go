// Package server 提供 LLM Speed Test 的 HTTP 服务：渠道配置（llm.yaml）CRUD API、
// 基准测试与历史记录 API，并可选托管前端构建产物（内嵌 embed.FS 或磁盘静态目录）。
//
// 既被 cmd/server 直接使用，也被统一入口 cmd/go-llm-speedtest 的 server 子命令复用，
// 保证两个入口的 HTTP 行为完全一致。
package server
...
import (
	...
	"github.com/SmilingXinyi/go-llm-speedtest/backend/internal/config"
	"github.com/SmilingXinyi/go-llm-speedtest/backend/internal/services"
)
...
func (s *Server) Start(addr string) error {
	...
	log.Printf("LLM Speed Test server listening on %s (config=%s, static=%s, history=%s)", addr, s.cfgPath, src, s.historyDir)
	return s.start(addr)
}
